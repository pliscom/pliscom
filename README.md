### Hi, I'm Philip 👋

I build with **data and AI** — LLM pipelines, AI agents, and full-stack apps - and I've spent 7+ years working with customers explaining and building technical tools.

I like sitting where engineering meets the customer: deep enough to build it, clear enough to explain why it matters.

🌏 Based in Australia · 💬 Happy to chat about data, AI, and solutions engineering

---

### 🛠️ Featured projects

**[picked-showcase](https://github.com/pliscom/picked-showcase)** — A taste-driven restaurant-discovery web app. Public case study covering the product thinking, architecture, and database design.
`Next.js 16` · `React 19` · `TypeScript` · `Supabase` · `Vercel`

**[finance-tracker](https://github.com/pliscom/finance-tracker)** — A shared personal-finance app — net worth, budgets, bills, and goals — with secure auth and row-level data isolation.
`Next.js` · `NextAuth` · `Supabase` · `TypeScript`

---

### 🧰 Working with

`SQL` · `TypeScript` · `Python` · `Next.js / React` · `Node.js` · `Supabase / Postgres` · `LLM pipelines & agents` · `Vercel`

---

### 📫 Get in touch

[![LinkedIn](https://img.shields.io/badge/LinkedIn-Philip%20Liscom-0A66C2?logo=linkedin&logoColor=white)](https://www.linkedin.com/in/philip-liscom-270290119/)
